...
def release2k8s(enviroment,service,configFile){
    ...
    try {
        sh """
            ...
            kubectl -n telecare apply -f ${service}-service.yml --kubeconfig=telecare-k8s-config
            kubectl -n telecare apply -f ${service}-ingress.yml --kubeconfig=telecare-k8s-config
            sleep 120
        """
...
def buildPushCommit() {
    ...
    if(env.gitlabBranch == env.STAGING_BRANCH){
        ...
        deploys['Deploy to K8s'] = {
            node("$env.node_slave"){
               stage("Deploy to K8s"){
                    release2k8s("k8s-test-performance","doctor","telecare-k8s-config")
               }
            }
        }
        parallel deploys
        ...
        stage("Deploy to K8s"){
            node("$env.node_slave"){
                release2k8s("k8s","doctor","telecare-k8s-config")
            }   
        }
    }
